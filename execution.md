...
#### 7.9 — Ham veri saklama, metrik doğrulaması, MVP metin temizliği

PO talebi üç parçalıydı: (1) müşteri tarafından yüklenen verinin gerçekten saklanıp saklanmadığının doğrulanması ve müşteri bazlı saklanması, (2) tüm metriklerin yeniden gözden geçirilmesi, (3) arayüzdeki "küçük notların" (dahili dosya/OQ atıfları, dev komutları) temizlenip ürünün MVP'ye hazır hale getirilmesi.

**(1) Ham işlem saklama — daha önce YOKTU.** `Assessment.ozellikler` yalnızca 9 türetilmiş özelliği (toplam gelir hacmi vb.) saklıyordu; yüklenen tek tek işlemler (tarih/kategori/tutar/açıklama) hiçbir yerde tutulmuyordu. PO kararı: müşterinin kendi yüklediği veri müşteri bazlı saklanmalı. Eklenenler:
- `Assessment.ham_islemler` (yeni `JSONField`, migration `0006`) — yalnızca kimliği doğrulanmış bir `Profil` varken doldurulur (bankanın demo/anonim skorlamalarında değil — o veri zaten kaynak CSV'de duruyor, ikinci kez saklamak kişisel veri ayak izini büyütmekten başka işe yaramaz). Yalnızca kanonik şema alanları (`tarih, islem_tipi, kategori, tutar, aciklama`) whitelist'lenerek yazılır.
- **Bulunan gerçek hata:** `VeriAgent.calistir()` kendisine verilen `islemler` listesinin her elemanına doğrudan `tarih_obj` (bir `datetime` nesnesi) yazıyordu — çağıran tarafın referansını sessizce mutate ediyordu. Bu değer JSON'a serileştirilemediğinden, ham veriyi saklamaya çalışan ilk istek `TypeError: Object of type datetime is not JSON serializable` ile patlıyordu (denetim yazımı best-effort olduğundan skorlama çökmüyordu ama kayıt hiç yazılmıyordu — sessiz veri kaybı). Düzeltme: `VeriAgent.calistir()` artık kopya üzerinde çalışıyor, çağıranın listesine dokunmuyor.
- Yeni uç: `GET /api/portal/gecmis/<id>` — müşterinin KENDİ geçmiş yüklemesinin tam detayını (saklanan işlemler dahil) döner; başka bir hesabın kaydına 404 (sızıntı yok). Frontend'de "Geçmişim" listesi artık açılır/kapanır — tıklanınca o yüklemenin tüm işlem tablosunu gösterir.
- `kimlik.Profil`'i olmayan bir hesabın (kurum personeli) bu uçlara erişememesi zaten §7.8'de düzeltilmişti (`ProfilSahibi`).
- Testler: `api/tests.py::HamIslemSaklamaTesti` (4 test) — portal yüklemesi saklıyor mu, müşteri kendi detayını görebiliyor mu, başkasının kaydına erişemiyor mu, demo/anonim skorlama saklamıyor mu.
- **Canlı doğrulama:** migration önce yalnızca `makemigrations` ile üretilmiş, geliştirme veritabanına `migrate` UYGULANMAMIŞTI — bu yüzden ilk canlı testte "Geçmişim" boş görünüyordu (API 500 dönüyordu, `_denetim_yaz`'ın best-effort `except Exception: pass`'i hatayı yutuyordu). `manage.py migrate` çalıştırıldıktan sonra tarayıcıda uçtan uca doğrulandı: gerçek PDF yüklendi, "87 işlem" olarak Geçmişim'de göründü, satır açılınca tam işlem tablosu (tarih/kategori/açıklama/tutar) render edildi.

**(2) Metrik yeniden doğrulaması.** `segmentasyon.py`/`genelleme_saglamlik.py`/`risk_istahi.py` yeniden çalıştırıldı (metin değişiklikleri aşağıda, madde 3) — **sayısal içerik birebir aynı** kaldığı `git diff` ile doğrulandı (yalnızca zaman damgası ve çalışma süresi değişti). `/api/metrikler`, `/api/segmentasyon`, `/api/adalet` canlı olarak sorgulanıp frontend'in gösterdiği sayılarla karşılaştırıldı — tutarlı. `aks_core` pytest (83) + Django (48, yukarıdaki 4 yeni dahil) + `tsc --noEmit` + `vite build` tekrar yeşil.

**(3) MVP metin temizliği.** Kullanıcıya görünen metinlerden `architecture.md §X` / `execution.md §X` / `OQ-NN` gibi dahili dosya atıfları ve `python manage.py bootstrap_kurum` / `python -m aks_core.model.X` gibi geliştirici komutları kaldırıldı (Intelligence/Portfolio/Audit/Customers/Kurum/Portal sayfaları + `api/services.py::_METRIK_UYARISI` ve üç rapor üreticisinin gömülü metinleri). **Bağlayıcı kural korundu:** CLAUDE.md'nin istatistiksel dürüstlük mandatı gereği döngüsel/doğrulanmamış veri uyarılarının İÇERİĞİ hiçbiri silinmedi — yalnızca dahili atıflar temizlenip düz, profesyonel dile çevrildi. Kurum giriş sayfasındaki demo-hesap komutu tamamen kaldırıldı (bir MVP'de son kullanıcıya shell komutu gösterilmez). OTP debug notu "demo modu" olarak yeniden çerçevelendi (SMS sağlayıcısı hâlâ yok, OQ-47 açık — ama artık "geçici/debug" değil kasıtlı bir demo özelliği gibi okunuyor).

**OQ-46 kısmen yanıtlandı:** PO bu turda "ham veri müşteri bazlı saklanmalı" kararını verdi — saklama SÜRESİZ olarak MVP seviyesinde uygulandı. Silme hakkı / KVKK'ye uygun saklama süresi politikası hâlâ açık (aşağıdaki tabloda not edildi).
...
#### 7.10 — Prod alt yapısına geçiş (Supabase/Upstash), Gemini tool-calling agent'ı, Redis dayanıklılığı

PO üç şey istedi: (1) `.env`'i gerçek Supabase/Upstash kimlik bilgileriyle doldurup canlıya hazır hale getirmek, (2) `ANTHROPIC_API_KEY` yerine şimdilik `GEMINI_API_KEY` ile çalışmak — ama "agent yapısı" (§7.5'in tool-calling + uydurma-sayı guard'ı) **korunarak**, eski tek-prompt zenginleştirme yoluna (`asistan.py::gemini_fonksiyonu`) düşülmeden, (3) tüm test paketinin baştan, gerçek doğrulamalarla yeşil olduğunun kanıtlanması.

**(1) Supabase Postgres.** `DATABASE_URL` dolduruldu (Session pooler URI, şifredeki `@` karakteri `%40` olarak encode edildi — aksi halde bağlantı stringi yanlış ayrıştırılıyordu). `migrate` gerçek Postgres'e karşı çalıştırıldı, canlı bir yazma (`POST /api/auth/kayit`) ile uçtan uca doğrulandı, sonra temizlendi. **Bilinen kalıcı ödünleşim:** Django test komutu her koşuda ayrı bir `test_postgres` veritabanı açıp yıkıyor — bu artık ağ üzerinden (Supabase EU-North), yerel SQLite'a göre çok daha yavaş (49 test ~170sn, öncesinde saniyeler). Ayrıca Supabase'in session pooler'ı bağlantıyı test sürecinden hemen sonra bırakmadığı için `manage.py test` sonunda `DROP DATABASE test_postgres` "is being accessed by other users" hatasıyla başarısız oluyor (**test sonuçlarını etkilemiyor** — `OK` zaten teardown'dan ÖNCE yazdırılıyor; yalnızca bir sonraki koşudan önce elle temizlik gerektiriyor, bkz. `pg_terminate_backend` + `DROP DATABASE IF EXISTS`).

**(2) Upstash Redis — canlıda bulunan gerçek bir hata.** `REDIS_URL` dolduruldu (`rediss://default:<token>@...upstash.io:6379` — REST API token'ı/URL'i DEĞİL, ayrı bir "Redis" bağlantı stringi; ilk denemede REST kimlik bilgileri yanlışlıkla `REDIS_URL`'e yazılacaktı, düzeltildi). Bağlantı test edilirken **geliştirme makinesinin sistem saatinin ileri tarihe (2026) kaymış olduğu** ve bunun Upstash'in gerçek TLS sertifikasını "süresi dolmuş" gibi gösterip her bağlantıyı reddettiği bulundu (`[SSL: CERTIFICATE_VERIFY_FAILED] certificate has expired`) — bu ortam sorunu, PO'nun düzeltmesi gerekiyor (sistem ayarı, ajan tarafından değiştirilemez).

Bu, teorik değil **gerçek bir prod-kırıcı bulgu** ortaya çıkardı: `CACHES` `REDIS_URL` doluyken koşulsuz `RedisCache`'e geçiyordu ve DRF'nin OTP/erişim-talebi throttle sınıfları (`kimlik/throttle.py`) `cache.get()`'i **hatasız çalışacağını varsayarak** çağırıyordu — Redis'e erişilemediğinde bu, ilgisiz uçları (`POST /api/kimlik/telefon/gonder`, `POST /api/kimlik/kurum/erisim-talebi`) **500 ile çökertiyordu**. Bunu `manage.py test` çalıştırılırken 4 gerçek test hatası olarak yakaladık (öncesinde `REDIS_URL` boş olduğu için hiç tetiklenmemişti). Düzeltme: `config/settings.py`'de `django_redis` `OPTIONS`'a `IGNORE_EXCEPTIONS: True` + `DJANGO_REDIS_IGNORE_EXCEPTIONS`/`DJANGO_REDIS_LOG_IGNORED_EXCEPTIONS` — Redis'e erişilemezse cache okuma/yazma **sessizce no-op** olur (loglanır, istisna fırlatmaz). Bilinçli ödünleşim: bu, Redis çökükken throttle limitlerinin de fail-open olacağı (sınırsız istek) anlamına gelir — kabul edilebilir, çünkü kullanılabilirlik burada kesintide mükemmel hız sınırlamadan daha önemli (django-redis'in resmi önerdiği desen). Aynı prensip zaten `AuditLog`'un best-effort yazımında ve kalibrasyon/anomali modellerinin opsiyonelliğinde var — yeni bir ilke değil, mevcut "opsiyonel bileşen çekirdek akışı asla düşürmemeli" kuralının bir uç noktaya daha uygulanması.

**(3) Gemini tool-calling agent'ı — `danisman_llm.py` çok-sağlayıcılı hale getirildi.** Önceki durum: `ANTHROPIC_API_KEY` yoksa `services.asistan_yanit()` doğrudan eski `asistan.py::AsistanAgent`'a düşüyordu — bu, tek bir prompt string'ine tüm bağlamı gömen, **araç yok, uydurma-sayı guard'ı yok** bir yoldu (bkz. §7.5'in "gerçek agent" iddiasının tam tersi). PO "Gemini kullanacağız, agent yapısı önemli" dediğinde bunun mevcut mimariyi (5 araç + `_dogrula` guard'ı) Gemini'ye de genişletmek anlamına geldiği netleşti — sadece eski yola düşmek değil.

Yapılan: `google-genai` SDK ile (`google.genai.Client`, `types.FunctionDeclaration`/`types.Tool`, function-calling round-trip) **canlı anahtarla önce ampirik olarak doğrulanan** (boş-parametreli araçlar + serbest-anahtarlı `degisiklikler` nesnesi dahil) bir `_yanitla_gemini()` eklendi — `AracKutusu` (5 aracın gerçek uygulaması) ve `_dogrula()` (uydurma-sayı guard'ı) **Claude yoluyla birebir paylaşılıyor**, yalnızca SDK/şema farklı (`ARAC_TANIMLARI` Anthropic formatı, `_gemini_arac_tanimlari()` Gemini formatı — elle senkron tutulmalı, yeni araç eklenirse ikisi de güncellenmeli). Sağlayıcı önceliği: `ANTHROPIC_API_KEY` varsa Claude, yoksa `GEMINI_API_KEY` varsa Gemini, ikisi de yoksa kural motoru. `services.py::asistan_yanit()` artık **her iki** anahtarı da kontrol ediyor (önceden yalnızca Anthropic'i kontrol edip Gemini'yi görmezden geliyordu — bu, ikinci bir gerçek hataydı, aşağıda). Yanıt sözleşmesine `saglayici: "anthropic"|"gemini"|"kural"` alanı eklendi (Agent İzi panelinde hangi sağlayıcının yanıtladığını göstermek için); `mod`/`anlati_reddedildi`/`arac_cagrilari` aynen korundu.

**Bulunan ikinci gerçek hata — test hijyeni:** Mevcut Django testi `test_asistan_ucu_anahtarsiz_kural_moduna_duser` yalnızca `ANTHROPIC_API_KEY`'in boş olduğunu doğruluyordu, `GEMINI_API_KEY`'i hiç kontrol etmiyordu. `.env`'e gerçek bir `GEMINI_API_KEY` girilince bu test **sessizce gerçek bir ağ çağrısı yapmaya başladı** (hermetik olmayan test) — `manage.py test`'in anormal yavaşlamasının kök nedeni buydu. Düzeltme: test artık `unittest.mock.patch.dict(os.environ, {"ANTHROPIC_API_KEY": "", "GEMINI_API_KEY": ""})` ile iki anahtarı da geçici olarak boşaltıyor (geliştiricinin gerçek `.env`'inde ne olursa olsun hermetik kalır). Ayrıca yeni bir regresyon testi eklendi (`test_asistan_ucu_anahtar_varsa_danisman_llm_e_delege_eder`) — bir anahtar varken isteğin gerçekten `danisman_llm.yanitla`'ya delege edildiğini (mock'lanmış, ağsız) doğruluyor. `aks_core/tests/test_danisman_llm.py`'ye Claude testleriyle birebir simetrik 6 yeni Gemini testi eklendi (başarılı araç çağrısı, uydurma-sayı reddi, senaryo_calistir, MAKS_TUR aşımı, API hatası, iki anahtar da varsa Claude'un önceliği).

**Canlı uçtan uca doğrulama** (gerçek `GEMINI_API_KEY` ile, `POST /api/asistan`): model `faktor_getir` ve `skor_getir` araçlarını gerçekten çağırdı, yanıt metnindeki tüm sayılar (610, 590) araç çıktılarıyla birebir eşleşti, `mod: "llm-arac"`, `saglayici: "gemini"`, `anlati_reddedildi: false` — OQ-48'in Gemini karşılığı artık kanıtlanmış durumda (Anthropic ile canlı test hâlâ açık, anahtar eklenince aynı yol otomatik devreye girer, kod değişikliği gerekmez).

`google-genai>=1.0` `requirements.txt`'e açıkça eklendi (önceden ortamda kurulu ama beyan edilmemiş bağımlılık deseni tekrarlanmasın diye — bkz. §7.1'in pdfplumber/pandas/openpyxl için aynı düzeltmesi). Frontend `api.ts::AsistanYanit` tipi gerçek backend sözleşmesiyle eşleştirildi (`mod: "llm-arac"|"kural"`, yeni `saglayici`/`arac_cagrilari` alanları — önceden `"llm"|"kural"` yazıyordu, `"llm-arac"` hiç yoktu, bu da ayrı, küçük bir tip-doğruluğu hatasıydı).

**Doğrulama:** `aks_core` pytest 89/89 (6 yeni Gemini testi dahil), Django `manage.py test` 49/49 (`OK`, Redis IGNORE_EXCEPTIONS sayesinde artık ağ hatasından bağımsız), `tsc --noEmit && vite build` temiz, canlı `/api/portfoy` (Redis kırıkken bile 200), canlı `/api/asistan` (gerçek Gemini tool-calling, yukarıda detaylı).

**Açık kalan:** Sistem saati düzeltilmeden Upstash Redis gerçekten kullanılamıyor (kod artık bunu tolere ediyor ama cache'in kendisi hâlâ devre dışı — in-memory'ye eşdeğer davranıyor, çok-worker/prod ortamında bu yetersiz kalır). `ANTHROPIC_API_KEY` hâlâ boş — eklenince Claude yoluna otomatik geçilir, ayrıca OQ-48'in Anthropic tarafının canlı testi gerekir.

#### 7.11 — Yüzey izolasyonu: banka içi araştırma arayüzünün yetkilendirilmesi (gerçek güvenlik açığı)

PO'nun tespiti: *"giren kullanıcı diğer kullanıcıların bilgilerini görmemeli, kurumlar sadece kendine verilenleri görebilmeli, kullanıcı sadece kendi içeriğini görmeli."* Denetim sonucu: portal (`api/portal_views.py`, `user=request.user` filtresi) ve kurum (`kimlik/kurum_views.py`, `KurumUyesi` + `aktif_riza()`) katmanları **zaten doğru kapsamlanmıştı** ve `test_kiracilik.py` bunu kanıtlıyordu. Açık, üçüncü yüzeydeydi.

**Bulgu 1 — `api/views.py`'nin tamamı izinsizdi.** Bu modül banka içi demo/araştırma yüzeyidir: tüm demo popülasyonunu (`/api/demo-musteriler`), portföy ve adalet toplu istatistiklerini, değerlendirme geçmişini gösterir. Hiçbir uçta `permission_classes` yoktu ve `REST_FRAMEWORK` içinde `DEFAULT_PERMISSION_CLASSES` tanımlı değildi — yani DRF varsayılanı `AllowAny` geçerliydi. Sonuç: **anonim bir istemci bile** bu uçların tamamına doğrudan erişebiliyordu; §7.10'da eklenen site geneli giriş kapısı yalnızca *arayüzü* gizliyor, API'yi korumuyordu. Düzeltme: yeni `kimlik/izinler.py::YoneticiKullanici` (`is_staff`) izni `bilgi` dışındaki 15 ucun tamamına eklendi. `is_staff` kasıtlı — yeni bir rol tablosu eklemek yerine Django'nun kendi yönetici bayrağı kullanıldı, çünkü bu yüzey zaten admin paneliyle aynı güven seviyesini gerektiriyor.

**Bulgu 2 — `services.gecmis()` çapraz yüzey sızıntısı, İKİ ayrı yoldan.** Fonksiyon yalnızca `musteri_id`'ye göre filtreliyordu. Portal yüklemeleri `musteri_id="-1"` ile yazılıyor (gerçek kimlik `user`/`profil` FK'sinde), yani birbirinden habersiz **tüm portal kullanıcıları bu tek anahtarı paylaşıyor** — tek bir çağrı hepsinin skor geçmişini tek listede döndürüyordu. İlk düzeltme sorguya `.exclude(kaynak="portal")` eklemekti; **yazdığım test bunun yetmediğini gösterdi.** DB sorgusu boş dönünce fonksiyon orkestratörün süreç-içi hafızasına (`Orkestrator.hafiza`, `{musteri_id: [...]}`) düşüyor ve portal/CSV skorlamaları oraya da aynı `-1` anahtarıyla yazıldığı için sızıntı **fallback yolundan aynen devam ediyordu**. Kalıcı düzeltme: `KIMLIKSIZ_MUSTERI_ID` sabiti tanımlandı ve fonksiyon bu değer için en baştan boş dönüyor — iki yol da kapalı. Test her iki yolu ayrı ayrı zorluyor (DB'ye portal kaydı yazıp hafızayı da kasten kirletiyor). Bu, "tek bir savunma katmanı ekleyip yeşile bakmanın" neden yetmediğinin somut örneği: filtre doğruydu ama sızıntının ikinci bacağını görünmez hale getiriyordu.

URL yolu (`<int:musteri_id>`) negatif değer kabul etmediği için bu uç dışarıdan bugün de erişilemiyordu — ama güvenlik, URL dönüştürücüsünün tesadüfi bir yan etkisine bırakılamaz; ikisi de ayrı testlerle sabitlendi.

**Bulgu 3 — kayıt akışında yetim hesap + yarış penceresi.** `kayit()` içinde `create_user()` başarılı olup `_profil_olustur()` patlarsa geriye AKS numarası olmayan, giriş yapabilen ama portal uçlarında `ProfilSahibi`'nden geçemeyen **kalıcı bozuk bir hesap** kalıyordu. Ayrıca `exists()` kontrolü ile `create_user()` arasındaki pencerede aynı e-postayla iki eşzamanlı kayıt `IntegrityError` → 500 üretiyordu. Düzeltme: ikisi tek `transaction.atomic()` bloğuna alındı, `IntegrityError` yakalanıp anlamlı 400'e çevrildi.

**Bulgu 4 — portal uçları `IsAuthenticated`'dı.** `api/portal_views.py`'nin üç ucu (`yukle`, `gecmis`, `gecmis/<id>`) yalnızca giriş yapılmış olmasını arıyordu, `Profil` varlığını değil. Sızıntı değil (hepsi `user=request.user` ile filtreli) ama AKS numarası olmayan bir kurum personeli hesabı belge yükleyip `profil=None` olan yarım bir `Assessment` üretebiliyordu. §7.8'de `kimlik/views.py` için zaten tanımlanmış olan `ProfilSahibi` iznine geçirildi — iki müşteri-taraflı modül artık aynı deseni kullanıyor.

**Rol bazlı yönlendirme.** `/api/auth/*` yanıtına `yonetici` (`is_staff`) ve `kurum_uyesi` bayrakları eklendi; `GirisPage` giriş/kayıt sonrası kullanıcıyı doğru yüzeye gönderiyor (yönetici → `/`, kurum personeli → `/kurum/musteriler`, sıradan kullanıcı → `/portal`) ve `Layout` yönetici olmayanı kendi yüzeyine geri yönlendiriyor. Bu bayraklar **yalnızca yönlendirme ipucudur** — yetki her uçta sunucuda zorlanır, tarayıcıda bayrağı taklit etmek 403'ten başka bir şey getirmez.

**Test/jüri hesapları.** `bootstrap_demo_hesaplar` management komutu üç hesabı idempotent olarak hazırlıyor (`ornek@aks.com` → portal, `admin@aks.com` → araştırma yüzeyi, `kurum@demo.aks` → kurum paneli; sonuncusu mevcut `bootstrap_kurum`'a devrediliyor, mantık kopyalanmadı). Aynı bilgiler giriş sayfasındaki kutucukların altında gösteriliyor — değiştirilirse ikisi birlikte değiştirilmeli.
...
#### 7.13 — Frontend'in "AKS Terminal" (Google Stitch) tasarımıyla tam değişimi

PO, `planning/stitch_aks_finansal_kapasite_platformu/` klasörüne 15 Stitch-üretimi statik HTML ekranı (+ `aks_terminal/DESIGN.md` tasarım sistemi) bıraktı ve frontend'in **tamamının** bu tasarımla değiştirilmesini istedi. Tüm 17 sayfa (15 Stitch karşılığı + PO kararıyla aynı sistemde kendim tasarladığım 2 sayfa: `AnaSayfaPage`, `GirisPage` — Stitch klasöründe bunların karşılığı yoktu), 3 `Layout` bileşeni, `tailwind.config.js`, `index.css` ve `Icon.tsx` yeniden yazıldı. Her mevcut sayfanın gerçek state/hook/API çağrı mantığı **birebir korunarak**, yalnızca JSX/markup/class'lar yeni tasarıma taşındı — bu bir reskin, davranış değişikliği değil.

**Tasarım sistemi konsolidasyonu.** 15 ekranın rengi/fontu/spacing'i normalize edilince (anahtar sırası hariç) birebir aynı çıktı — tek kaynak olarak `tailwind.config.js`'e taşındı. Ama her ekranın kendi inline `<style>` bloğu birbirinden bağımsız üretildiği için aynı kavram (örn. "yükseltilmiş kart yüzeyi") farklı isimler taşıyordu (`panel-l1`/`tonal-level-1`/`bg-level-1`/`card-layer`), bazı ekranlar body arkaplanını config'in `background` token'ından (#131317) sapıp hardcoded #020617 kullanıyordu. Hepsi `index.css`'te TEK, anlam bazlı isimlendirmeyle konsolide edildi (`hairline-border`, `caveat-banner`, `shap-positive/negative`, `zebra-row`, `agent-log`, `file-drag-over`, `pulse-dot`) — böylece 17 sayfa arasında görsel tutarlılık var, kaynak ekranların kendi aralarındaki tutarsızlık miras alınmadı.

**Uydurma veri KULLANILMADI.** Stitch mockup'larının çoğu gerçekçi görünmesi için sahte içerik taşıyordu — uydurma müşteri isimleri (Ahmet Yılmaz, Zeynep Kaya), sahte kurum adları (FinansBank A.Ş.), sahte "0x8f4a2b..." imza/hash dizeleri, "blokzincir altyapısı" gibi YANLIŞ bir teknik iddia (gerçek güvence Django'nun append-only DB kısıtı, blokzincir değil), sabit/statik sonuç sayıları (538 kurtarılan, 13.4M TL). Her sayfa dönüştürülürken bunlar tek tek ayıklanıp gerçek API alanlarıyla (`PERSONA_ETIKET`, gerçek `risk_seviyesi`, gerçek `t.kurum`/`t.amac` vb.) değiştirildi — projenin "sayı uydurma yok" ilkesi (`_dogrula` guard'ıyla aynı ruh) frontend'e de uygulandı. `/audit` sayfasında isim çakışması da vardı: Stitch'in "Denetim Defteri" ekranı literal bir AuditLog tablosuydu (çapraz-müşteri listeleme ucu backend'de yok, uydurmak gerekirdi) — bunun yerine mevcut gerçek fairness/model-validity/segmentasyon/sağlamlık içeriği aynı tasarım diliyle korundu.

**Fonksiyonel iyileştirmeler (kapsam içi, mockup'ın kendisi tetikledi).** Portföy sayfasındaki eşik kaydırıcıları (mockup'ta süsti, hiçbir yere bağlı değildi) gerçekten `api.portfoy(esikler)`'i yeniden çağırıyor — backend zaten `klasik_esik`/`aks_esik` query param'larını destekliyordu, frontend hiç kullanmıyordu (`api.ts::portfoy()` imzası genişletildi). Telefon doğrulama artık 6 kutulu gerçek OTP girişi (otomatik odak ilerletme/backspace), tek metin kutusu değil. Portal mobil nav'ında mockup'ın unuttuğu "Rıza Defterim" sekmesi eklendi (3/4 öğe vardı, dördü de gerekli).

**Doğrulama (kod düzeyi):** her sayfa yazıldıktan hemen sonra `tsc --noEmit` (17/17 temiz), sonunda `npm run build` temiz, üç demo hesabıyla (yönetici/kullanıcı/kurum) canlı curl akışı (giriş, `/api/bilgi`, `/api/demo-musteriler`, `/api/skorla/<id>`, `/api/csv-skorla`, `/api/kimlik/*`, `/api/kimlik/kurum/*` — hepsi beklenen şemayı döndürdü), tüm 17 rota Vite dev sunucusunda 200, kullanılmayan import taraması, eski tasarım sisteminin (`font-body-sm`, `ai-glow`, `glass-panel` vb.) sıfır kalıntı bıraktığının grep ile teyidi. Backend'e HİÇ dokunulmadı (`git status` ile doğrulandı) — yine de regresyon güvencesi için Django 66/66 ve `aks_core` pytest 89/89 tekrar koşuldu, ikisi de yeşil.

#### 7.14 — Uçtan uca tarayıcı denetimi: reskin sonrası bulunan 5 hata

Reskin tamamlandıktan sonra tarayıcı eklentisi bağlandı ve 17 ekranın tamamı gerçek oturumlarla, gerçek verilerle gezildi (yönetici → kullanıcı → kurum). Kod düzeyi doğrulamanın (tsc/build/curl/test) **yakalayamadığı** beş hata çıktı — hepsi ancak ekranda görülünce fark edilebilecek türden:

| # | Nerede | Neydi | Kök neden |
|---|---|---|---|
| 1 | `config/settings.py` (backend) | Panel gezinirken tüm uçlar 500 dönmeye başlıyordu: `FATAL: (EMAXCONNSESSION) max clients reached ... pool_size: 15` | `conn_max_age=60` yetersizdi. §7.11'de uçlar yetkilendirildiğinden beri HER korumalı istek oturum doğrulaması için DB'ye gidiyor — "DB'ye dokunmayan uç" kalmadı. Tarayıcı bir sayfada 5 paralel istek atınca dev sunucusu 5 thread'de 5 bağlantı açıp 60 sn tutuyor, Supabase ücretsiz katmanın 15 istemci kotası birkaç sayfada doluyordu. `conn_max_age=0`'a çekildi (pooler arkasında kalıcı bağlantı tutmak anti-pattern; havuzlama zaten pooler'ın işi). 20 ardışık + 8 paralel istekle doğrulandı: 0 hata. |
| 2 | `PortfolioPage` / `api.ts` | "Simülasyonu Çalıştır" üstteki özet kartlarını güncelliyor ama alttaki segment oranları/tablosu ESKİ eşikte kalıyordu | `api.adalet()` eşik parametrelerini göndermiyordu (backend zaten kabul ediyordu). `api.portfoy()` ile aynı imzaya getirildi. |
| 3 | `tailwind.config.js` | Müşteri detayındaki twin-score daireleri KARE çıkıyordu | Stitch'in 15 ekran export'u `borderRadius.full: "0.75rem"` yazıyor ama kendi `DESIGN.md`'si `full: 9999px` diyor ("Full pill (999px) radius"). Export kendi tasarım belgesiyle çelişiyordu; DESIGN.md kanonik kabul edildi. |
| 4 | `CustomerDetailPage`, `KurumMusteriDetayPage` | SHAP değerleri `+-0.722` gösteriyordu (çift işaret) ve kırmızı çubuklar konteynerin dışına, ekranın soluna taşıyordu | `f.etki` backend'den ZATEN işaretli geliyor (`riski_azaltan` negatif, `riski_artiran` pozitif) — elle `+`/`-` eklenmişti. Ayrıca Stitch'in `absolute right-full` deseni çubuğu konteyner dışına itiyordu; iki sayfa da mockup'ın kendi ikiye-bölme diverging desenine hizalandı. |
| 5 | `CustomersPage` | Sabit konumlu "Etkinlik Günlüğü" paneli tablonun RİSK BANDI/AKSİYON kolonlarını kapatıyordu | `fixed` + `pointer-events-none` tıklamayı geçiriyordu ama içerik okunamıyordu. Panel normal akışa, tablonun altına alındı. |

**Uçtan uca doğrulanan senaryolar (tarayıcıda, gerçek veriyle):** yönetici girişi → panel metrikleri (AUC 0.840, %95 GA, 2.000 müşteri, ONLINE) · portföy eşik simülasyonu (1015→1079 kurtarılan, net −48.750→−351.750 TL, segment oranları da güncellendi) · denetim defteri (adalet/model-geçerliliği/segmentasyon/R8-R10-R11/agent denetimi) · müşteri listesi + detay (twin-score, SHAP, PD-Gap, agent izi) · what-if simülatörü (fatura düzeni 0→0,5 → skor 832→835) · **AKS Asistanı canlı Gemini tool-calling** (yanıttaki skor ve faktörler backend SHAP çıktısıyla birebir uyuştu) · yüzey izolasyonu (kullanıcı hesabıyla `/panel` → portala geri, `/kurum/*` → kurum girişine) · telefon OTP (6 kutulu giriş, otomatik odak ilerletme, gerçek doğrulama) · **tam rıza döngüsü**: kurum talep açtı → müşteri portalinde göründü → onayladı (30 gün) → rıza defterine iki olay yazıldı → kurum müşteriyi listede ve detayda gördü (3 risk profili: 835/760/690) → müşteri iptal etti → kurum ANINDA 403 aldı ve liste boşaldı.
...
| OQ-46 | User portal (§3b Phase 6) now accepts real end-user-uploaded statements tied to a real account — this makes DA5 (KVKK/consent/lawful-basis governance, already TODO before this phase) more urgent. What's the minimum before accepting *real* (non-demo) user data: explicit consent checkbox + privacy notice text only, or a full data-retention/deletion policy + DPA-style documentation? | `PortalPage.tsx`, `PortalLoginPage.tsx`, DA5 | 🟡 PARTIALLY RESOLVED (§3b Phase 7/7.9) — PO decided raw uploaded transactions ARE retained per-customer, indefinitely, at MVP level (`Assessment.ham_islemler`). Still open: a real retention-duration / right-to-deletion policy and DPA-style documentation — nothing auto-expires or lets a customer purge their history today. |
