...
// Tasarım: planning/stitch_aks_finansal_kapasite_platformu/aks_denetim_defteri
// ("AKS Terminal" — hairline-border kartlar, zebra-row tablolar, lock ikonlu
// değiştirilemezlik vurgusu). Stitch mockup'ı literal bir "denetim kaydı"
// tablosuydu (sabit/uydurma satırlar, "CST-8921-X" gibi) — ama bunun için
// çapraz-müşteri bir liste ucu backend'de yok (`/api/gecmis/<id>` tek
// müşteriye özel). Uydurma satırlar göstermek projenin "sayı uydurma yok"
// ilkesini (bkz. `_dogrula` guard'ı, execution.md) ihlal ederdi. Bu yüzden
// GERÇEK içerik (adalet/model-geçerliliği/segmentasyon/sağlamlık/agent
// denetimi — hepsi gerçek API'lere bağlı) korunuyor, yalnızca görsel dil
// Stitch'in "değiştirilemez defter" estetiğine (hairline border, mono
// etiketler, kilit ikonu) taşınıyor.
const AGENT_AUDIT = [
  ...
  return (
    <div className="flex flex-col gap-stack-default">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="font-headline-lg text-headline-lg text-on-surface mb-2 flex items-center gap-2">
            ...
            Değiştirilemez Denetim Defteri
          </h1>
          <p className="font-body-md text-body-md text-on-surface-variant max-w-2xl">
            AKS'nin equal-opportunity metriğiyle gruplar arası davranışını, model geçerliliğini ve agent
            dürüstlüğünü gösterir. Bu bir yasal görüş değildir.
          </p>
        </div>
        <div className="flex items-center gap-2 bg-surface-container border border-outline-variant p-2">
          <div className="flex flex-col items-end px-3">
            <span className="font-mono-label-sm text-mono-label-sm text-[10px] text-on-surface-variant uppercase">Kaynak</span>
            <span className="font-mono-label-sm text-mono-label-sm text-primary">/api/adalet</span>
          </div>
          <div className="h-8 w-px bg-outline-variant" />
          <div className="flex flex-col items-end px-3">
            <span className="font-mono-label-sm text-mono-label-sm text-[10px] text-on-surface-variant uppercase">Durum</span>
            <span className="font-mono-label-sm text-mono-label-sm text-secondary flex items-center gap-1">
              <span className={`w-2 h-2 rounded-full ${hata ? "bg-error" : "bg-secondary pulse-dot"}`} />
              {hata ? "HATA" : "CANLI"}
            </span>
          </div>
        </div>
      ...
      {adalet?.veri_kaynagi === "dongusel" && (
        <div className="border caveat-banner text-caveat p-3 font-body-md text-body-md">
          <span className="font-semibold">Not:</span> {adalet.uyari}
        </div>
      )}

      <div className="grid grid-cols-1 md:grid-cols-12 gap-gutter">
        {/* Adalet Parity Gauge */}
        <section className="md:col-span-4 bg-surface-container-low border border-outline-variant p-6 relative flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h2 className="font-headline-md text-headline-md text-on-surface">Adalet Parity</h2>
              <p className="font-mono-label-sm text-mono-label-sm text-on-surface-variant">AKS eşit-fırsat parity</p>
            </div>
          ...
          <div className="relative py-12 flex flex-col items-center justify-center">
            <div className="relative w-48 h-48">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="font-mono-label-sm text-mono-label-sm text-secondary">PARITY</span>
              </div>
            </div>
            <div className="mt-8 grid grid-cols-2 gap-4 w-full">
              <div className="flex flex-col">
                <span className="font-mono-label-sm text-mono-label-sm text-[10px] text-on-surface-variant uppercase">AKS Boşluğu</span>
                <span className="font-headline-md text-headline-md text-on-surface">
                  {adalet ? adalet.aks_skor.equal_opportunity_boslugu.toFixed(3) : "—"}
                </span>
              </div>
              <div className="flex flex-col">
                <span className="font-mono-label-sm text-mono-label-sm text-[10px] text-on-surface-variant uppercase">Klasik Boşluğu</span>
          ...
          <div className="bg-surface-container-highest/30 p-3 mt-4 border border-outline-variant">
        ...
        <section className="md:col-span-8 bg-surface-container-low border border-outline-variant p-6 flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-secondary/10 flex items-center justify-center text-secondary border border-secondary/20">
                <Icon name="groups" />
              </div>
              <div>
                <h2 className="font-headline-md text-headline-md text-on-surface">Equal Opportunity Monitor</h2>
                <p className="font-mono-label-sm text-mono-label-sm text-on-surface-variant uppercase tracking-wider">
              ...
              return (
                <div className="space-y-2" key={persona}>
                  <div className="flex justify-between items-end">
                    <span className="font-body-md text-body-md text-on-surface">{PERSONA_ETIKET[persona] ?? persona}</span>
                    <span className="font-mono-label-sm text-mono-label-sm text-primary">
                      {oran.toFixed(2)} Parity · n={g.n}
                    </span>
                  </div>
                  <div className="h-2 w-full bg-surface-container-highest overflow-hidden flex">
        ...
        <section className="md:col-span-5 bg-surface-container-low border border-outline-variant p-6 flex flex-col">
          <div className="flex items-center gap-2 mb-4">
            <Icon name="psychology" className="text-on-surface-variant" />
            <h2 className="font-headline-md text-headline-md text-on-surface">Reason Code Inspector</h2>
          </div>
          <div className="flex items-center gap-2 mb-6">
            <input
              ...
              className="bg-surface border border-outline-variant rounded-DEFAULT px-3 py-2 font-mono-data-md text-mono-data-md text-on-surface w-28 focus:outline-none focus:border-primary"
            />
            <button
              onClick={incele}
          ...
          {incelemeHata && <p className="text-error font-mono-label-sm text-mono-label-sm mb-2">{incelemeHata}</p>}
          {inceleme && (
            <div className="space-y-2 overflow-y-auto pr-2 flex-grow max-h-[360px]">
              ...
              {inceleme.aciklama.riski_azaltan.map((f) => (
                <div className="p-3 zebra-row border-l-2 border-shap-positive" key={f.kod}>
                  <div className="flex justify-between mb-1">
                    <span className="font-mono-label-sm text-mono-label-sm text-shap-positive">{f.kod}</span>
                    <span className="font-mono-label-sm text-mono-label-sm text-on-surface-variant">{f.etki.toFixed(3)}</span>
                  </div>
                  <p className="font-body-md text-body-md text-on-surface">{f.faktor} — riski azaltıyor</p>
                </div>
              ))}
              {inceleme.aciklama.riski_artiran.map((f) => (
                <div className="p-3 zebra-row border-l-2 border-shap-negative" key={f.kod}>
                  <div className="flex justify-between mb-1">
                    <span className="font-mono-label-sm text-mono-label-sm text-shap-negative">{f.kod}</span>
                    <span className="font-mono-label-sm text-mono-label-sm text-on-surface-variant">+{f.etki.toFixed(3)}</span>
                  </div>
                  <p className="font-body-md text-body-md text-on-surface">{f.faktor} — riski artırıyor</p>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Boundary Integrity */}
        <section className="md:col-span-7 bg-surface-container-low border border-outline-variant overflow-hidden">
          <div className="p-4 flex items-center justify-between border-b border-outline-variant">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-secondary" />
              <h2 className="font-mono-label-sm text-mono-label-sm text-on-surface font-bold uppercase tracking-widest">
          ...
          <div className="p-6 flex flex-col gap-4">
            <p className="font-body-md text-body-md text-on-surface-variant leading-relaxed">
              ...
              denetim satırı (<code className="font-mono-data-md text-mono-data-md">AuditLog</code>) üretir.
            </p>
            <div className="grid grid-cols-2 gap-4 font-mono-label-sm text-mono-label-sm">
        ...
        <section className="md:col-span-12 bg-surface-container-low border border-outline-variant p-6">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              <Icon name="query_stats" className="text-primary" />
              <h2 className="font-headline-md text-headline-md text-on-surface">Model Validity</h2>
            </div>
            {metrikler && (
              <span className="font-mono-label-sm text-mono-label-sm text-[10px] text-on-surface-variant uppercase">
          ...
          {metriklerHata && (
            <div className="border border-error/40 bg-error-container/20 text-error p-3 font-mono-label-sm text-mono-label-sm mb-4">
          ...
          {metrikler && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-gutter">
              {metrikler.modeller.map((m) => (
                <div key={m.ad} className="border border-outline-variant bg-surface-container-lowest p-4">
                  <div className="font-mono-label-sm text-mono-label-sm text-on-surface font-bold mb-3">{m.ad}</div>
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div>
                      <div className="text-[10px] text-on-surface-variant uppercase font-mono-label-sm">ROC-AUC</div>
                      <div className="font-headline-md text-headline-md text-primary">{m.roc_auc.ortalama.toFixed(3)}</div>
                      <div className="text-[10px] text-on-surface-variant font-mono-label-sm">
                        %95 CI [{m.roc_auc.ci95[0].toFixed(3)}, {m.roc_auc.ci95[1].toFixed(3)}]
                      </div>
                    </div>
                    <div>
                      <div className="text-[10px] text-on-surface-variant uppercase font-mono-label-sm">ECE (kalibrasyon)</div>
                      <div className="font-headline-md text-headline-md text-secondary">{m.ece_oof.toFixed(3)}</div>
                      <div className="text-[10px] text-on-surface-variant font-mono-label-sm">Brier {m.brier_oof.toFixed(3)}</div>
                    </div>
                  </div>
                  {Object.keys(m.persona_metrik).length > 0 && (
                    <div className="space-y-1">
                      <div className="text-[10px] text-on-surface-variant uppercase font-mono-label-sm mb-1">Persona bazında AUC</div>
                      {Object.entries(m.persona_metrik).map(([persona, pm]) => (
                        <div key={persona} className="flex justify-between font-mono-label-sm text-mono-label-sm text-[11px]">
        ...
        <section className="md:col-span-12 bg-surface-container-low border border-outline-variant p-6">
          <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-headline-md text-headline-md text-on-surface">Segmentasyon (Denetimsiz Keşif)</h2>
            </div>
            {segmentasyon && (
              <span className="font-mono-label-sm text-mono-label-sm text-[10px] text-on-surface-variant uppercase">
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-gutter mb-4">
                {Object.entries(segmentasyon.kume_profilleri)
                  .sort(([, a], [, b]) => b.n - a.n)
                  .map(([kume, prof]) => (
                    <div key={kume} className="border border-outline-variant bg-surface-container-lowest p-4">
                      <div className="flex justify-between items-baseline mb-3">
                        <span className="font-mono-label-sm text-mono-label-sm text-on-surface font-bold">Küme {kume}</span>
                        <span className="font-mono-label-sm text-[10px] text-on-surface-variant">n={prof.n}</span>
                      </div>
                      <div className="mb-3">
                        <div className="text-[10px] text-on-surface-variant uppercase font-mono-label-sm">Ampirik temerrüt oranı</div>
                        <div className="font-headline-md text-headline-md text-secondary">{(prof.temerrut_orani * 100).toFixed(1)}%</div>
                      </div>
                      <div className="text-[10px] text-on-surface-variant uppercase font-mono-label-sm mb-1">Persona dağılımı</div>
                      <div className="space-y-1">
              ...
              <p className="font-mono-label-sm text-[10px] text-on-surface-variant leading-relaxed">{segmentasyon.not}</p>
            </>
          )}
        </section>
        ...
        <section className="md:col-span-12 bg-surface-container-low border border-outline-variant p-6">
          <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-headline-md text-headline-md text-on-surface">Genelleme &amp; Sağlamlık</h2>
            </div>
            {genelleme && (
              <span className="font-mono-label-sm text-mono-label-sm text-[10px] text-on-surface-variant uppercase">
          ...
          {genelleme && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-gutter">
              ...
              <div className="border border-outline-variant bg-surface-container-lowest p-4">
                <div className="font-mono-label-sm text-mono-label-sm text-on-surface font-bold mb-1">
                  R8 — Persona-dışı genelleme
                </div>
                <p className="font-mono-label-sm text-[10px] text-on-surface-variant mb-3">
                ...
                <div className="pt-3 border-t border-outline-variant">
                  <div className="text-[10px] text-caveat uppercase font-mono-label-sm mb-1">Out-of-time split</div>
                  <p className="font-mono-label-sm text-[10px] text-on-surface-variant leading-relaxed">
              ...
              <div className="border border-outline-variant bg-surface-container-lowest p-4">
                ...
                <table className="w-full font-mono-label-sm text-[11px]">
                  <thead>
                  ...
                  <tbody>
                    {Object.entries(genelleme.ince_dosya_stres_testi.sonuc).map(([k, s]) => (
                      <tr key={k} className="border-t border-outline-variant/50">
                        ...
                        <td className="py-1 text-right text-caveat">
              ...
              <div className="border border-outline-variant bg-surface-container-lowest p-4">
                ...
                <div className="space-y-2">
                  {Object.entries(genelleme.oyunlanabilirlik_duyarliligi.sonuc)
                    ...
                    .map(([feat, s]) => (
                      <div key={feat}>
                        <div className="flex justify-between font-mono-label-sm text-mono-label-sm text-[11px] mb-0.5">
                        ...
                        <div className="h-1.5 w-full bg-surface-container-highest overflow-hidden">
                          <div
                            className={`h-full ${s.ort_skor_kazanci > 20 ? "bg-error" : "bg-secondary"}`}
        ...
        <section className="md:col-span-12 bg-surface-container-low border border-outline-variant p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="font-headline-md text-headline-md text-on-surface">Agent Beş-Soru Denetimi</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-gutter">
            {AGENT_AUDIT.map((a) => (
              <div
                key={a.ad}
                className={`p-4 bg-surface-container-lowest border transition-all ${
                  a.gecti ? "border-secondary/50" : "border-outline-variant"
                }`}
              >
                <div className="flex justify-between items-start mb-3">
                  <span
                    className={`font-mono-label-sm text-mono-label-sm text-[10px] px-1.5 py-0.5 rounded-DEFAULT ${
                ...
                <div className="font-mono-label-sm text-mono-label-sm text-on-surface font-bold mb-2">{a.ad}</div>
                <p className="font-body-md text-body-md text-on-surface-variant">{a.verdict}</p>
              </div>
            ))}
          </div>
        </section>
